...
export const discoveryRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.get<{ Querystring: { limit?: string } }>(
    ...
    async (req, reply) => {
      ...
      try {
        ...
        const vLoc = await app.prisma.$queryRawUnsafe<
  ...
  app.get<{ Params: { profileId: string } }>(
    ...
    async (req, reply) => {
      ...
      const vLocRows = await app.prisma.$queryRawUnsafe<
        Array<{ lat: number; lng: number }>
      >(
        `SELECT ST_Y(location::geometry) AS lat, ST_X(location::geometry) AS lng FROM "Profile" WHERE "userId" = $1`,
        req.userId!,
      );
      const cLocRows = await app.prisma.$queryRawUnsafe<
        Array<{ lat: number; lng: number }>
      >(
      ...
      const hoursSince = Math.floor(
        (Date.now() - candidate.lastActiveAt.getTime()) / 3_600_000,
      );
      ...
      const explanation = explain(
        {
          ...
          preferences: {
            ...
            includeUnansweredOptionalFields:
              viewer.preferences.includeUnansweredOptionalFields,
          },
        },
        {
          profile: {
